  ...
  for (int iteration = 0; iteration < iter; iteration++) {
    ...
    for (a = 0; a < vocab_size; a++){
      ...
      for (b = 0; b < layer1_size; b++) {
        ...
        if (nor_max_u < nor_base_u) nor_max_u = nor_base_u;
        if (nor_min_u > nor_base_u) nor_min_u = nor_base_u;
        if (nor_max_v < nor_base_v) nor_max_v = nor_base_u;
        if (nor_min_v > nor_base_v) nor_min_v = nor_base_u;
      }
      
      for (b = 0; b < layer1_size; b++) {
        u[a*layer1_size+b] = ((u[a*layer1_size+b] - nor_min_u) / (nor_max_u - nor_min_u)) * 2 - 1;
