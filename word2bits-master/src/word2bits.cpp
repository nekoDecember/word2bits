...
void *TrainModelThread(void *id) {
  ...
  */
    ...
    //local_reg_loss += cur_val_nor * cur_val_nor;
	}
...
void TrainModel() {
  ...
  for (int iteration = 0; iteration < iter; iteration++) {
    ...
    for (a = 0; a < vocab_size; a++){
      ...
      nor_long_u = sqrt(nor_long_u);
      nor_long_v = sqrt(nor_long_v);
      for (b = 0; b < layer1_size; b++) {
        u[a*layer1_size+b] = u[a*layer1_size+b] / nor_long_u;
        v[a*layer1_size+b] = v[a*layer1_size+b] / nor_long_v;
      }
      }
    //kokomade
